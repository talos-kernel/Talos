...
# These pins are part of the installer, never read from the unverified archive.
# Wheel-only installation also prevents an unpinned source-build dependency chain.
# BEGIN PINNED VERIFIER
cat > "$TMP/verifier.lock" <<'VERIFIER_LOCK'
# Generated from requirements.lock by scripts/sync-verifier-lock.py.
...
VERIFIER_LOCK
# END PINNED VERIFIER
spin "verifier deps" "$TMP/verify/bin/python" -m pip install --quiet --require-hashes --only-binary=:all: --no-deps -r "$TMP/verifier.lock" \
