...
TOOL_PROTOCOL = (
    ...
    '- computer_run {"op":"browser","project":"slug","key":"unique-step-key","title":"short goal","action":"navigate","url":"https://…"} — persistent browser, works headless too. Other actions: inspect; fill/type/select(selector,value); check(selector,checked); click/submit/wait(selector); press(selector,value); upload(selector,path inside project). Optional frame is an observed iframe selector. Reuse the observed tab identifier across actions; numeric page indices can reorder after reconnect. Never send tab and page together. All actions keep project/key/title. Every inspect field carries a guard hash; pass it unchanged as "expect" on the next action against that field — if the element changed since the inspect, the action refuses instead of hitting the wrong element.\n'
    "For forms use browser navigate then inspect: its receipt lists actual fields, selectors, "
    "labels, required state, options, guards and validation errors. Use those observed selectors, "
    ...
    "Receipts include the request arguments so you can tell which action produced an output. "
    "Once a read-back verifies the requested result, finish with the answer. Do not repeat "
    "a successful write to verify it again; use a read-only check if evidence is still missing.\n"
